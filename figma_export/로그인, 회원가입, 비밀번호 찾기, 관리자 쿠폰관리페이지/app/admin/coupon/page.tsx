...
import { Plus, Tag, Eye, Edit, Trash2, Download, CheckCircle, Clock, BarChart3, DollarSign, Search } from "lucide-react"
...
export default function AdminCouponPage() {
  ...
  const [filterType, setFilterType] = useState<"all" | "active" | "expired" | "used">("all")
  const [filteredCoupons, setFilteredCoupons] = useState<Coupon[]>(sampleCoupons)
                    ...
                    onClick={() => {
                      ...
                      if (searchQuery) {
                        filtered = filtered.filter((coupon) => {
                          const matchesName = coupon.name.toLowerCase().includes(searchQuery.toLowerCase())
                          const matchesUser = couponUsage
                            .filter((usage) => usage.couponId === coupon.id)
                            .some((usage) => usage.userName.toLowerCase().includes(searchQuery.toLowerCase()))
              ...
              <CardContent className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <button
                    ...
                    className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 rounded-lg hover:from-blue-100 hover:to-blue-200 transition-all duration-200 text-left"
                  >
                    <div className="flex items-center">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const activeOnly = sampleCoupons.filter((c) => c.status === "active")
                      setFilteredCoupons(activeOnly)
                    }}
                    className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-lg hover:from-green-100 hover:to-green-200 transition-all duration-200 text-left"
                  >
                    <div className="flex items-center">
                  ...
                  <button
                    onClick={() => {
                      setFilterType("used")
                      const usedOnly = sampleCoupons.filter((c) => c.usedQuantity > 0)
                      setFilteredCoupons(usedOnly)
                    }}
                    className="bg-gradient-to-r from-yellow-50 to-yellow-100 p-4 rounded-lg hover:from-yellow-100 hover:to-yellow-200 transition-all duration-200 text-left"
                  >
                    <div className="flex items-center">
                    ...
                    {filterType === "all"
                      ...
                      : filterType === "active"
                        ...
                        : filterType === "expired"
                          ? "만료된 쿠폰"
                          : "사용된 쿠폰"}
                    <span className="ml-2 text-sm text-gray-500">({filteredCoupons.length}개)</span>
                  </CardTitle>
                  <Button variant="outline" size="sm" className="flex items-center bg-transparent">
                    <Download className="w-4 h-4 mr-1" />
                    내보내기
